...
// TrollCoin: optional setting to unlock wallet for block minting only;
...
// TrollCoin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // TrollCoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // TrollCoin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // TrollCoin: not avatar mode
                    {
...
// TrollCoin: create coin stake transaction
...
// TrollCoin: check 'spent' consistency between wallet and txindex
// TrollCoin: fix wallet spent state according to txindex
...
// TrollCoin: disable transaction (only for coinstake)
